...
def preprocess(sentence):
    ...
    ''.join(filter(lambda x: x in printable, sentence))
...
for i in range(1,10001):
    ...
    tsvname = "Clean_Mv3tsv_tpi_"+str(i)+".tsv"
...
import math
def tfidf(doc_id,term_id):
    #for i in range(1,10):# will add more files
    file = doc_id
    file1 = open(file, encoding="utf8") 
    line = file1.read() 
    words = line.split('\t') 
    ...
    word=list(dictionary_3.keys())[list(dictionary_3.values()).index(term_id)]
    tf=L.count(word)/len(L)
    idf=math.log(10000/float(len(dic3[dictionary_3[word]])))
    tfidf=tf*idf
    return(tfidf)
...
dic_tfidf=dict( zip(range(len(dictionary_3)),[[] for i in range(len(dictionary_3))]))
...
for i in dic3.items():
    for j in i[1]:
        dic_tfidf[int(i[0])].append((j,tfidf(j,i[0])))
...
with open('dfidf.json', 'w') as fp:
    json.dump(dic_tfidf, fp)


# In[477]:


with open(r"dfidf.json", 'r') as file:
    data = file.read()
    tfidf_dic = json.loads(data)
tfidf_dic
...
def tfidf_query(query):
    ...
    for x in text:
        try:
            idx=dictionary_3[x]
            tf_query=text.count(x)/len(text)
            idf_query=math.log(N/len(dic3[str(idx)]))
            tfidf_qry[idx]=tf_query*idf_query
        except:
            print("xxxx")
            continue
    return (tfidf_qry)
...
tfidf_query('disney movie')


# ####  Getting all tf-idf information about the query
...
def info(query):
    ...
    for x in text:
        try:
            idx=dictionary_3[x]
            tif_info[idx]=tfidf_dic[idx]
        except:
            print("Limited database error")
            continue
    return(tif_info)
